...
class libPixels {
    ...
    async fetchRecentGameLib() {
        ...
        const versionResponse = await fetch('/version.json?nhud=true');
        ...
        const headers = { 
            ...
            'User-Agent': `${currentUA} NerdHUDNext` 
        };
        
        // fetch game library
        let response = await fetch(`${this.pixelsAPIURL}/game/library?tenant=${this.pixelsTenant}&ver=${clientVersion}&nhud=true`, { headers });
        let game_lib = await response.json();

        // fetch ugc library
        response = await fetch(`${this.pixelsAPIURL}/game/library?tenant=${this.pixelsTenant}&ugc=true&ver=${clientVersion}&nhud=true`, { headers });
    ...
    async fetchLanguageData() {
        ...
        const response = await fetch('https://pixels-server.pixels.xyz/v1/i18n/game/en?nhud=true', { headers });
    ...
    _onPixelsMessage(type, data) {
        ...
        if (type == "sellOrders" && data.str_merchantContracts) {
            window.postMessage({
                type: 'contracts',
                data: data.str_merchantContracts
            })
        }
    ...
    detectEntityChanges(e_old, e) {
        ...
        if (e.entity.startsWith("ent_mine_")) {
            ...
            if ((e_old.generic.current == 0) && (e.generic.current != 0)) {
                window.postMessage({
                    type: 'mine_picked',
                    data: flattenEntity(e)
                })
        ...
        if (e.entity.startsWith("ent_playersoil")) {
            if ((!e_old.generic.utcRefresh)  && e.generic.utcRefresh >= Date.now()) {
                window.postMessage({
                    type: 'event_crop_planted',
                    data: flattenEntity(e)
                })
            }
            if (e_old.generic.state == "grown" && e.generic.state != "grown") {
                window.postMessage({
                    type: 'event_crop_picked',
                    data: flattenEntity(e)
                })
            }
        }
