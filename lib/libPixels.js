...
class libPixels {
    constructor() {
        ...
        this.state_change_throttle = 250; // Maximum delay in milliseconds
        this.state_change_last_invoke = 0;
        this.latest_room_state = null;
        this.latest_player_state = null;
        this.state_change_timer = null;
    ...
    _onRoomStateChange(input_data) {
        // Update room state from input or fallback
        this.latest_room_state = JSON.parse(JSON.stringify(input_data));
        this.latest_player_state = null;
        this._scheduleStateChange();
    }

    _onPlayerStateChange(input_data) {
        // Update player state from input or fallback
        this.latest_room_state = null;
        this.latest_player_state = JSON.parse(JSON.stringify(input_data));
        this._scheduleStateChange();
    }

    _scheduleStateChange() {
        const now = Date.now();
        const timeSinceLastInvoke = now - this.state_change_last_invoke;

        // If enough time has passed, invoke immediately
        if (timeSinceLastInvoke >= this.state_change_throttle) {
            this._invokeStateChange();
        } 
        // Otherwise, schedule the next invocation
        else if (!this.state_change_timer) {
            this.state_change_timer = setTimeout(() => {
                this.state_change_timer = null;
                this._invokeStateChange();
            }, this.state_change_throttle - timeSinceLastInvoke);
        }
    }

    _invokeStateChange() {
        this.state_change_last_invoke = Date.now();

        // Ensure room_state and player_state fall back if missing
        const room_state = this.latest_room_state 
            ? this.latest_room_state 
            : JSON.parse(JSON.stringify(this.pixels_scene.stateManager.room.serializer.state));

        const player_state = this.latest_player_state 
            ? this.latest_player_state 
            : JSON.parse(JSON.stringify(this.getSelfPlayer()));
    ...
    _onStateChange (room_state, player_state) {
        ...
        for (let i in player_state.entities) {
            try {
                ...
                console.log("onStateChange: Error when merging entities for comparison: ", player_state.entities[i], room_state.entities[player_state.entities[i].mapEntity_id])
            }
        }
        ...
        if (this.room_state) {
            ...
            for (let i in entities) {
                ...
                if ((e_old.generic?.trackers.lastTimer < e.generic?.trackers.lastTimer) && ((e.generic?.trackers.lastTimer > server_time) || (e.generic?.utcRefresh > server_time) || ((e_old.generic?.state == 0) && (e.generic?.state != 0)) )) {
                    window.postMessage({
                        type: 'entity_started',
                        data: e
                    })
