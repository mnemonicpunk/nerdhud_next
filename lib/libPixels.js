...
class libPixels {
        ...
        this.interceptPhaserObject("Phaser.Game", (game_instance) => {
            ...
            let _scene_update_tick = () => {
                ...
                if (_self.pixels_scene == null) {
                    ...
                    console.log("STORAGES TO SEND: ", this.storages);
                    for (let i in this.storages) {
                        let s = this.storages[i];
                        console.log("SENDING STORAGE: ", s)
                        window.postMessage({
                            type: 'set_storage',
    ...
    _onPixelsMessage(type, data) {
        console.log("PIXELS MESSAGE: ", type, data);
    ...
    _onStateChange (room_state, player_state) {
        ...
        if (this.room_state) {
            ...
            for (let i in entities) {
                ...
                if ((e_old.generic?.trackers.lastTimer < e.generic?.trackers.lastTimer) && ((e.generic?.trackers.lastTimer > Date.now()) || (e.generic?.utcRefresh > Date.now()) || ((e_old.generic?.state == 0) && (e.generic?.state != 0)) )) {
                    window.postMessage({
                        type: 'entity_started',
                        data: e
                    })
    ...
    setTrackedStorage(entity) {
        console.log("TRACKING STORAGE: ", entity);
