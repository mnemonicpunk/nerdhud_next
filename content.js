...
class NerdHUD {
    ...
    async loadEnvironment(url) {
        try {
            ...
            chrome.storage.local.get([libPixelsKey], async (result) => {
                ...
                if (isVersionNewer(install.libPixels.version, currentVersion)) {
                    ...
                    const fileUrl = resolveBuiltinPath(install.libPixels.url);
                    ...
                    chrome.storage.local.set({
                        ...
                        if (chrome.runtime.lastError) {
                            console.error(`Failed to update Chrome storage: ${chrome.runtime.lastError.message}`);
                        } else {
                            console.log(`Updated to version ${install.libPixels.version} successfully.`);
                        }
                    });
                    ...
                    this.loadApps(install.apps, (apps) => {
                        /*const [DebugApp, NerdMenuApp, StorageApp, TaskboardApp, StatsApp, TimerApp, InventoryApp, MarketApp, ItemInfoApp] = apps;
            
                        this.apps.push(new InventoryApp(this));
                        this.apps.push(new MarketApp(this));
                        this.apps.push(new DebugApp(this));
                        this.apps.push(new NerdMenuApp(this));
                        this.apps.push(new TaskboardApp(this));
                        this.apps.push(new StatsApp(this));
                        this.apps.push(new ItemInfoApp(this));
                        this.apps.push(new StorageApp(this));
                        this.apps.push(new TimerApp(this));*/
    ...
    loadApps(filenames, callback, progress_callback) {
        // Create an array of promises for each dynamic import
        let total = filenames.length;
        let completed = 0;
        const imports = filenames.map(filename => {
          const fileUrl = resolveBuiltinPath(filename); // Resolving to chrome-extension:// URL
    ...
    loadAppData(mid, callback) {
        ...
        chrome.storage.local.get("NHN_" + mid, function(result) {
            if (chrome.runtime.lastError) {
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                chrome.storage.local.set(dataToSave, function() {
                    if (chrome.runtime.lastError) {
                        console.error("Error saving data for mid:", mid);
                        console.dir(chrome.runtime.lastError);
                        resolve(false);
                    } else {
                        console.log("Data saved for mid:", mid);
                        resolve(true);
                    }
                });
            });
